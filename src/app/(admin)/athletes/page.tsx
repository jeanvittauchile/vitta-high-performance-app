  ...
  return (
    <div style={{ padding: '20px 24px 28px' }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-end', marginBottom: 16 }}>
        <div>
          <div style={{ fontSize: 11, letterSpacing: '0.1em', textTransform: 'uppercase', color: 'var(--text-muted)', fontWeight: 700 }}>Workspace · Atletas</div>
          <div className="display" style={{ fontSize: 28, fontStyle: 'italic' }}>{ATHLETES.length} atletas activos</div>
        </div>
        <button className="btn btn-primary"><PlusIcon size={13}/>Invitar atleta</button>
      </div>
          ...
          {filtered.map(a => {
            ...
            return (
              <div key={a.id} onClick={() => router.push(`/athletes/${a.id}/planner`)} className="lift" style={{
                ...
                <div style={{ width: 44, height: 44, borderRadius: 22, background: cat?.color, color: '#fff', display: 'grid', placeItems: 'center', fontSize: 13, fontWeight: 700 }}>{a.initials}</div>
